...
class PIMQuantizedRuntime:
    ...
    def __init__(self, *, profile: str = "", rank_count: int = 1, instance_key: str = "") -> None:
        ...
        # ADR-002 M-24 Stage A: C-level async submit + wait.
        # submit_async returns an opaque token (c_void_p) immediately so
        # Python can proceed with GPU work; wait joins the pthread worker
        # and surfaces any error.  The underlying fused op still performs
        # all PIM matvecs; Stage A only changes WHEN Python blocks.
        self._lib.pim_quantized_run_many_fused_silu_async.argtypes = [
            ctypes.c_void_p,   # handle_gate_up
            ctypes.c_void_p,   # handle_down
            ctypes.c_uint32,   # call_count
            ctypes.POINTER(ctypes.c_uint32),  # gate_up_batch_sizes
            ...
            ctypes.POINTER(ctypes.c_void_p),  # down_outputs
            ctypes.c_uint32,   # down_output_dim
            ctypes.POINTER(ctypes.c_void_p),  # out_token
            ctypes.c_char_p,
            ctypes.c_size_t,
        ]
        self._lib.pim_quantized_run_many_fused_silu_async.restype = ctypes.c_int
        self._lib.pim_quantized_fused_wait.argtypes = [
            ctypes.c_void_p,   # token
            ctypes.c_char_p,
            ctypes.c_size_t,
        ]
        self._lib.pim_quantized_fused_wait.restype = ctypes.c_int
    ...
    @staticmethod
    def submit_many_fused_silu_async(
        gate_up_runtime: "PIMQuantizedRuntime",
        down_runtime: "PIMQuantizedRuntime",
        requests: list[tuple[
        ...
        """ADR-002 M-24 Stage A: submit fused gate_up+silu*up+down to a
        C-level pthread worker and return immediately with an opaque handle.

        The handle owns the preallocated output tensors + ctypes arrays;
        the caller MUST call ``handle.wait()`` exactly once to:
          * join the worker thread,
          * raise on any C-level error,
          * collect the list of down output tensors.

        Crucially, all torch tensors and ctypes arrays remain alive via
        the handle's own references until wait() completes, so the C
        worker's raw pointers stay valid across the async window.
        """
        if not requests:
            return PIMFusedAsyncHandle(
                lib=gate_up_runtime._lib,
                token=None,
                outputs=[],
                inputs_alive=[],
                arrays_alive=[],
                gate_up_runtime=gate_up_runtime,
                down_runtime=down_runtime,
                n=0,
            )
        if gate_up_runtime is None or down_runtime is None:
            raise ValueError("submit_many_fused_silu_async requires both runtimes")
        lib = gate_up_runtime._lib
        n = len(requests)

        # Same prepare loop as the sync fused path.  We keep every tensor
        # referenced so Python GC cannot free its storage before the
        # worker reads it.
        gate_up_inputs_prepared: list[torch.Tensor] = []
        down_outputs: list[torch.Tensor] = []
        gate_up_batch_sizes: list[int] = []
        ...
        for req in requests:
            ...
            if inputs.ndim != 2:
                raise ValueError("submit_many_fused_silu_async expects 2D inputs.")
            if gate_cols != up_cols:
                raise ValueError(
                    f"gate_cols ({gate_cols}) must equal up_cols ({up_cols}) for SwiGLU"
                )
            ...
            if down_output_dim == 0:
                ...
                raise ValueError(
                    "fused_async: all requests must share the same down padded_output_dim"
                )
        ...
        out_token = ctypes.c_void_p(0)
        rc = lib.pim_quantized_run_many_fused_silu_async(
            gate_up_runtime._handle,
            down_runtime._handle,
            ctypes.c_uint32(n),
            gu_batch_arr,
        ...
        # Keep everything referenced by the handle so nothing is GC'd
        # during the async window.
        arrays_alive = [
            gu_batch_arr, gu_slot_arr, gu_input_ptrs,
            gate_cols_arr, up_cols_arr,
            dn_batch_arr, dn_slot_arr, dn_output_ptrs,
        ]
        return PIMFusedAsyncHandle(
            lib=lib,
            token=out_token,
            outputs=down_outputs,
            inputs_alive=gate_up_inputs_prepared,
            arrays_alive=arrays_alive,
            gate_up_runtime=gate_up_runtime,
            down_runtime=down_runtime,
            n=n,
        )
...
class PIMFusedAsyncHandle:
    """Opaque handle returned by ``PIMQuantizedRuntime.submit_many_fused_silu_async``.

    Encapsulates:
      * the C-level pthread ``token`` (opaque void*),
      * the preallocated output tensors that will be filled by the worker,
      * ``inputs_alive`` / ``arrays_alive`` references that keep all raw
        pointers passed to C valid for the lifetime of the async op.

    ``wait()`` must be called exactly once per non-empty submit; it blocks
    until the worker completes, raises on any C-level error, and returns
    the list of output tensors.  Calling ``wait()`` twice raises a clear
    RuntimeError rather than double-joining the pthread.
    """
    ...
    def __init__(
        ...
        *,
        ...
        token,
        outputs: list[torch.Tensor],
        inputs_alive: list[torch.Tensor],
        arrays_alive: list,
        gate_up_runtime: PIMQuantizedRuntime,
        down_runtime: PIMQuantizedRuntime,
        n: int,
        ...
        self._inputs_alive = inputs_alive
        self._arrays_alive = arrays_alive
        self._gate_up_runtime = gate_up_runtime
        self._down_runtime = down_runtime
        self._n = n
        self._completed = False

    def wait(self) -> list[torch.Tensor]:
        if self._completed:
            raise RuntimeError("PIMFusedAsyncHandle.wait() called twice")
        self._completed = True
        if self._n == 0 or self._token is None:
            # Empty submit — nothing to wait on.  Still call fused_wait()
            # on a non-null sentinel if the C layer returned one, so the
            # sentinel job struct gets freed.
            if self._token is not None and int(self._token.value or 0) != 0:
                error_buffer = ctypes.create_string_buffer(
                    PIMQuantizedRuntime.ERROR_BUFFER_SIZE
                )
                rc = self._lib.pim_quantized_fused_wait(
                    self._token, error_buffer, len(error_buffer)
                )
                if rc != 0:
                    raise RuntimeError(
        ...
        if rc != 0:
            raise RuntimeError(error_buffer.value.decode("utf-8", errors="replace"))
        # Record native profile for both runtimes (the fused worker called
        # pim_quantized_run_many on each ctx, stamping last_* fields).
        self._gate_up_runtime._record_run_profile(count=self._n)
        if self._down_runtime is not self._gate_up_runtime:
            self._down_runtime._record_run_profile(count=self._n)
        return self._outputs

    def __del__(self) -> None:
        # Defensive: if user forgot to wait(), the pthread may still be
        # running with our buffers alive.  We join synchronously here as
        # a last resort, swallowing any error (best-effort cleanup).
        if not self._completed and self._token is not None:
            try:
                error_buffer = ctypes.create_string_buffer(
                    PIMQuantizedRuntime.ERROR_BUFFER_SIZE
                )
                self._lib.pim_quantized_fused_wait(
                    self._token, error_buffer, len(error_buffer)
                )
            except Exception:
                pass
