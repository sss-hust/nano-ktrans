...
| M-8 | 🚧 下一个 (CRITICAL) | **Handle-based host_quantized_bridge.c 重构**: 把 ~20 个 C `static` 全局 (g_set, g_initialized, g_input_dim, g_slot_loaded_mask, ...) 封进 `pim_q_ctx_t*` 结构体; Python 每 runtime 持 `c_void_p` handle. 做完**一次真机跑**就能一口气验证 M-5 dual / M-6 multi-slot / M-7 per-layer 三者叠加效果. 预期 hit ratio 15-30%, decode_tps 0.30 → 0.40-0.55. |
