...
| M-11 | 🚧 下一个 | **选项 A**: C-level `dpu_launch(DPU_ASYNCHRONOUS)` + 整层 batch launch (消除 Python-C 1488 roundtrip, 预期 tps 0.29 → 0.40-0.50). **选项 B**: 系统扫 offload_device_experts ∈ {2,16,32,48,64} × prompt length 的 OOM 边界; 如果 stable 推 32 作为新默认. 两者并行. |
