...
| M-9 | 🚧 下一个 | **先量化再优化**: (1) `--pim-layer-group-size` CLI 暴露 sweep; (2) `jaccard(topk_t, topk_t-1)` routing locality histogram; (3) `dpu_launch(DPU_ASYNCHRONOUS)` 把协调开销 overlap 掉. 不再盲目投资 caching 路径. |
