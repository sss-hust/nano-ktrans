...
| **M-7** | ✅ **PASS (8/8, 第 4 个 null perf + 诊断 milestone)** | Python 层做 per-layer-group scoping (默认 group_size=3) + GPTQ speculative preload (默认 OFF). **一跑 32 token e2e 直接 munmap_chunk heap corruption**, 紧急排查揭示**贯穿 M-5/M-6/M-7 的底层 bug**: `libpim_quantized_bridge.so` 的 ~20 个 `static` C 全局让所有 Python runtime **物理共享同一个 DPU rank pool**。M-5 dual / M-6 multi-slot / M-7 per-layer 的 Python 诊断 47/48 distinct 都是 Python 层假象, 底层从未分离。降级 speculative_preload_gptq=False 后 M-7 能稳定跑 (0.309 vs M-6 0.300, 噪声内), null perf 闭合, 但**精准锁定了 M-8 的唯一修复路径**。|
| **M-8** | ✅ **PASS (9/9, 真隔离 landed + null perf)** | **Handle-based `host_quantized_bridge.c` 完成**: 20 个 `static` 全局封进 `pim_q_ctx_t*`, 13 个导出函数加 `void *handle` 首参. 真机 sanity **首次证明独立 runtime 真分到不同 DPU rank pool**. 项目史上**首次观测到 preload_hits_local > 0 (24 次)**. 但 decode_tps 0.242 (vs M-7 0.309, **-22%**) 因 32 独立 rank pool 的 UPMEM driver 协调开销 ~1.3 ms/call + **Qwen3 routing temporal locality 仅 0.1%**（vs ADR §15.7 预估 20-30%, 低 100 倍）. 第 5 个 null perf milestone, 但揭示了真实瓶颈是 MoE routing distribution 本身. |
| **M-9** | ✅ **PASS (11/11, 数据驱动决策闭合)** | 加 `--pim-layer-group-size` CLI + Jaccard routing-locality histogram diagnostic. 5 个 group_size sweep + 1 final = 6 artifacts. 数据: Qwen3 top_k=8 decode Jaccard 均值 **0.14** (45.7% 样本 <10% 重叠), group_size=48 最快 (tps=0.290 vs gs=3 tps=0.246, -15%). **决定性结论**: slot-based caching 在 Qwen3 上理论上限 14% 但被 32-rank-pool 协调开销吃掉 10×. 默认翻回 group_size=48 + speculative_preload_gptq=False. `decode_tps = 0.2844` vs M-8 的 0.242 **+18%**. M-5~M-8 infra 保留但默认关, high-locality 场景可手动启用. |
| **M-10** | ✅ **PASS (10/10, 第 6 个 null perf + 1 个意外胜利)** | Python `threading.Thread` async submit infra + telemetry landed. A/B 实测 @ offload=2: async ON 0.271 vs OFF 0.284 (-4.7%); @ offload=32: async ON 0.340 vs OFF **0.3506** (-3.1%). Python GIL 争用 + 线程开销 ~5ms/call × 1488 call 吞掉 overlap 收益. Default 翻回 async OFF. **意外副产品**: `offload_device_experts=32 + async OFF` 跑出 **0.3506 tps 超 M-4 peak 0.317 +10.6%** — weight residency 旋钮被严重低估. |
| **M-11** | ✅ **PASS (10/10, 真正大胜利)** | Residency sweep 找到 `offload_device_experts=88` 安全默认。M-11 final decode_tps **0.6226**，相对 M-9 0.284 **+119%**，相对 M-4 peak 0.317 **+96%**。offload=94 在 short/medium 更快但 long prompt OOM，95/96 short OOM，因此默认选 88 保留显存余量。|
| M-12 | 🚧 下一个 | 扩展 OOM envelope (`offload ∈ {88,90,92,94}` × prompt length × max_new_tokens) + 在 offload=88 新 baseline 上做 C-level batched/async DPU launch。|
...
|-----------|------|-------|---------|------|
| **M-1** 基线建立 | 1-2 天 | e2e GPTQ 跑通 + 全 shape×batch×rank×mode sweep | 可信对照数据 + 量化 mode=6 是否 fake | ✅ **2026-04-22 PASS**（6/6 gate rules） |
| **M-2** 真 T-MAC kernel | 1 周 | `kernel_mode=7`（weight bit-sliced LUT） | ≥ mode=4 × 1.3，`batch=4/8` 不掉 0.9× 以下 | 🟡 **下一步**（gate WAIT） |
